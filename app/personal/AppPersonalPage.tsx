...
export default function AppPersonalPage({ xTestDb }: { xTestDb?: string }) {
    ...
    const [personalInfo, setPersonalInfo, loading] = usePersonal(xTestDb);
        ...
        try {
            ...
            const response = await fetch(toUrl('/api/personal'), {
                ...
                headers: {
                    ...
                    ...(xTestDb ? { 'x-test-db': xTestDb } : {})
                },
                body: JSON.stringify({ type, value: payload })
            });
            if (response.ok) {
    ...
    const persistCareerGoals = async (payload: PersonalInformationCareerGoal[]) => {
        setSaving(true);
        ...
        try {
            const response = await fetch(toUrl('/api/personal'), {
                ...
                body: JSON.stringify({ type: 'career_goals', value: payload })
            });
            if (!response.ok) {
